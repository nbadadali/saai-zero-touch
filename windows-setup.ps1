...
# Register (or refresh) the Scheduled Task that wakes WSL shortly after logon.
# The generated script remains active until the gateway and n8n ports respond,
# preventing Task Scheduler from reporting success before WSL actually starts.
...
$stackBody = @'
# Auto-generated by windows-setup.ps1. Wakes WSL2 at logon and waits until the
# OpenClaw gateway and n8n are listening before reporting task success.
$ErrorActionPreference = 'Stop'
...
try {
  ...
  Log ('wake requested (distro=' + $distro + ')')

  # Avoid `sh -c "sleep 600"`: Start-Process flattens ArgumentList and can pass
  # `sleep` without its duration. --exec supplies each Linux argument directly.
  $keeper = Start-Process -PassThru -WindowStyle Hidden -FilePath $wslExe `
    -ArgumentList @('-d', $distro, '--exec', 'sleep', '600')
  Log ('WSL keepalive started (pid=' + $keeper.Id + ')')
  Start-Sleep -Seconds 3
  if ($keeper.HasExited) {
    Log ('ERROR: WSL keepalive exited immediately (exit=' + $keeper.ExitCode + ')')
    exit 1
  }
  ...
  $n8nReady = $false
  do {
    $gatewayReady = Test-TcpPort 18789
    $n8nReady = Test-TcpPort 5678
    if ($gatewayReady -and $n8nReady) { break }
    Start-Sleep -Seconds 5
  } while ((Get-Date) -lt $deadline)

  if (-not ($gatewayReady -and $n8nReady)) {
    Log ('ERROR: readiness timeout (gateway=' + $gatewayReady + ', n8n=' + $n8nReady + ')')
    exit 1
  }
  Log 'READY: OpenClaw gateway and n8n are listening'
  exit 0
} catch {
...
Register-ScheduledTask -TaskName $autoTaskName -Action $autoAction -Trigger $autoTrigger -Settings $autoSettings -Principal $principal -Description "OpenClaw: wake WSL2 after logon and verify gateway+n8n readiness" | Out-Null
Write-Ok "Scheduled task registered: $autoTaskName (fires 15 s after logon and verifies UI ports)"
...
Start-Process -WindowStyle Hidden -FilePath 'wsl.exe' -ArgumentList @('-d', $Distro, '--exec', 'sleep', '600')
