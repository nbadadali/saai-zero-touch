...
# Create a robust wrapper script that waits for Docker to be ready before
# starting the stack. This fixes the race condition where docker compose up
# runs before dockerd has fully initialised on slow/cold boots.
$wrapperScript = Join-Path $launcherDir "stack-start.sh"
# Use single-quoted here-string (@'...'@) so PowerShell does NOT expand bash $ variables.
# $autoStartScript is substituted via string concatenation outside the here-string.
$wrapperBody = '#!/usr/bin/env bash
# Auto-generated by windows-setup.ps1
# Waits for Docker daemon to be ready, then starts the stack.
set -uo pipefail
LOGFILE="$HOME/.openclaw-autostart.log"
exec >>$LOGFILE 2>&1
echo "--- autostart $(date) ---"

# Wait up to 120 s for Docker daemon
WAIT=0
until docker info &>/dev/null; do
  if [[ $WAIT -ge 120 ]]; then
    echo "ERROR: Docker daemon not ready after 120 s -- aborting"
    exit 1
  fi
...
# Write with LF line endings so bash inside WSL can read it
$wrapperBodyLF = $wrapperBody -replace "`r`n", "`n"
$wrapperBytes  = [System.Text.Encoding]::UTF8.GetBytes($wrapperBodyLF)
[System.IO.File]::WriteAllBytes($wrapperScript, $wrapperBytes)
Write-Ok "Stack wrapper script: $wrapperScript"

# Make the wrapper executable inside WSL
wsl -d $WslDistro -u $WslUser -- chmod +x ($wrapperScript -replace "\\", "/" -replace "C:", "/mnt/c") 2>$null | Out-Null

# Scheduled task: fires 90 seconds after logon, runs the Docker-aware wrapper.
# 90 s is enough for WSL networking without being as long as the old 2-min delay.
...
# Convert Windows path to WSL path for the wrapper
$wrapperWslPath = ($wrapperScript -replace "\\", "/") -replace "^([A-Za-z]):", { "/mnt/$($_.Value.ToLower().TrimEnd(':'))" }
...
$autoTrigger.Delay = "PT1M30S"   # 90-second delay after logon
$autoSettings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -ExecutionTimeLimit (New-TimeSpan -Minutes 15)
Register-ScheduledTask -TaskName $autoTaskName -Action $autoAction -Trigger $autoTrigger -Settings $autoSettings -RunLevel Highest -Description "OpenClaw: wait for Docker readiness then bring up WSL stack at logon" | Out-Null
Write-Ok "Delayed scheduled task registered: $autoTaskName (fires 90 s after logon, waits for Docker)"
