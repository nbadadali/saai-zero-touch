...
# Write a Docker-readiness wrapper script into the launcher folder.
# Build line-by-line to avoid PowerShell here-string escaping issues.
$wrapperScript  = Join-Path $launcherDir "stack-start.sh"
# WSL path: convert C:\Users\<name>\AppData\Local\... -> /mnt/c/Users/<name>/AppData/Local/...
$wrapperWslPath = "/mnt/c/Users/$env:USERNAME/AppData/Local/OpenClaw/stack-start.sh"
$scriptLines = @(
  '#!/usr/bin/env bash',
  '# Auto-generated by windows-setup.ps1',
  '# Waits for Docker then starts the stack.',
...
[System.IO.File]::WriteAllText($wrapperScript, $scriptContent, [System.Text.Encoding]::UTF8)
wsl -d $WslDistro -u $WslUser -- chmod +x $wrapperWslPath 2>$null | Out-Null
Write-Ok "Stack wrapper: $wrapperScript"
