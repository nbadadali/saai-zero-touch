...
# Scheduled task: simply starts WSL 90 seconds after logon.
# Once WSL is running, systemd auto-starts OpenClaw (user service + linger)
# and Docker auto-restarts all containers (restart: unless-stopped policy).
# No wrapper script needed --the cascade is fully automatic.
...
$autoAction   = New-ScheduledTaskAction -Execute "wsl.exe" -Argument "-d $WslDistro -u $WslUser -- echo 'WSL started'"
$autoTrigger  = New-ScheduledTaskTrigger -AtLogOn
$autoTrigger.Delay = "PT1M30S"   # 90 s --enough for Windows networking to settle
...
Register-ScheduledTask -TaskName $autoTaskName -Action $autoAction -Trigger $autoTrigger -Settings $autoSettings -RunLevel Highest -Description "OpenClaw: start WSL 90 s after logon --systemd and Docker restart policies do the rest" | Out-Null
