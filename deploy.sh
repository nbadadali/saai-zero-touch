...
phase_gateway() {
  ...
  # Gateway authentication is mandatory even on loopback. Persist one stable
  # token so CLI diagnostics, the Control UI, and restarts all use the same
  # credential instead of a different runtime-only token on every startup.
  local gateway_token auth_json bin
  gateway_token="${OPENCLAW_GATEWAY_TOKEN:-}"
  if [[ -z "${gateway_token}" ]]; then
    gateway_token="$(gen_secret)"
    [[ -n "${gateway_token}" ]] || die "OpenClaw gateway token generation failed."
    set_env_var "OPENCLAW_GATEWAY_TOKEN" "${gateway_token}" "${CONFIG_FILE}"
    chmod 600 "${CONFIG_FILE}"
    ok "generated and persisted OpenClaw gateway token in config.env"
  fi

  auth_json="$(jq -nc --arg token "${gateway_token}" '{mode:"token",token:$token}')"
  openclaw config set gateway.mode local
  openclaw config set gateway.auth "${auth_json}" --strict-json
  openclaw config validate
  ok "gateway token authentication configured"
...
phase_browser() {
  ...
  local win_host_ip cdp_url profile_json gateway_token doctor_output cdp_ready=false _i current_list updated_list
  win_host_ip="$(ip route show default 2>/dev/null | awk '$0 !~ /docker0|br-|veth/ {print $3; exit}')"
  ...
  # Phases run in subshells, so a token generated by the earlier gateway phase
  # must be re-read from config.env before authenticating this CLI request.
  gateway_token="${OPENCLAW_GATEWAY_TOKEN:-}"
  if [[ -z "${gateway_token}" ]]; then
    gateway_token="$(bash -c "source '${CONFIG_FILE}' 2>/dev/null; printf '%s' \"\${OPENCLAW_GATEWAY_TOKEN:-}\"" 2>/dev/null || true)"
  fi
  [[ -n "${gateway_token}" ]] || die "OpenClaw gateway token is missing. Re-run: ./deploy.sh --from gateway"

  if doctor_output="$(openclaw browser --token "${gateway_token}" --browser-profile "${OPENCLAW_BROWSER_PROFILE}" doctor 2>&1)"; then
