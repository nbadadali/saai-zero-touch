...
phase_wsl_config() {
  ...
  cat > "${_bt}" <<BOOTEOF
#!/bin/sh
# Auto-generated by deploy.sh — force-start the user systemd manager at WSL boot.
n=0
until systemctl start "user@${uid}.service" 2>/dev/null; do
  n=\$((n+1)); [ "\$n" -ge 10 ] && break; sleep 3
done
BOOTEOF
  sudo mkdir -p /usr/local/bin
  sudo install -m 0755 "${_bt}" /usr/local/bin/saai-boot.sh
  rm -f "${_bt}"
  ok "boot helper installed: /usr/local/bin/saai-boot.sh (starts user@${uid}.service)"
